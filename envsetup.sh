...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^kami_") ; then
       CUSTOM_BUILD=$(echo -n $1 | sed -e 's/^kami_//g')
    else
       CUSTOM_BUILD=
    fi
    export CUSTOM_BUILD
...
function breakfast()
{
    ...
    for f in `/bin/ls vendor/kami/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the kami model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch kami_$target-$variant
        fi
    fi
    return $?
}
